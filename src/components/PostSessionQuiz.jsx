...
async function generateQuestions(subject, topic, studiedText) {
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: `You are a quiz generator for students. Generate exactly 4 multiple choice questions to test understanding.

Subject: ${subject}
Topic: ${topic || "General concepts"}
What the student studied: ${studiedText}

Rules:
- Questions must be directly based on what the student said they studied
...
export default function PostSessionQuiz({ subject, topic, sessionId, onComplete, onDismiss }) {
  // phases: "input" | "loading" | "quiz" | "results"
      ...
      <div className="w-full max-w-[480px] mx-auto flex flex-col min-h-screen">
        ...
        <header className="flex items-center justify-between px-5 py-4 sticky top-0 bg-[#f9faf3] z-10 border-b border-[#c7c5d4]/20">
          ...
          {phase !== "loading" && (
            <button
              onClick={onDismiss}
              className="w-9 h-9 rounded-full bg-[#e8e9e2] flex items-center justify-center active:scale-95 transition-transform"
            >
          ...
          <div className="flex-1 px-5 py-6 flex flex-col gap-6">
            ...
            <div className="bg-[#5150b1] rounded-[28px] p-6 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="text-[14px] text-white/80 mt-1">
                  Let's do a quick quiz to lock in what you learned.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                placeholder="e.g. I studied derivatives, chain rule, and product rule. Covered how to differentiate composite functions..."
              ...
              />
              <p className="text-[11px] text-[#464552] px-1">
                The more detail you give, the better the questions will be.
              </p>
            </div>
        ...
        {phase === "loading" && (
          <div className="flex-1 flex flex-col items-center justify-center gap-6 px-5">
            ...
            <div className="text-center">
              ...
              <p className="text-[14px] text-[#464552] mt-1">Claude is crafting questions just for you</p>
            </div>
            <div className="flex gap-2 mt-4">
              {[0, 1, 2].map((i) => (
                <div
                  key={i}
                  className="w-2 h-2 rounded-full bg-[#5150b1] animate-bounce"
          ...
          <div className="flex-1 px-5 py-4 flex flex-col gap-4">
            ...
            <div className="space-y-1.5">
              <div className="w-full h-2.5 bg-[#e8e9e2] rounded-full overflow-hidden">
                <div
                  className="h-full bg-[#5150b1] rounded-full transition-all duration-500"
                  style={{ width: `${progress}%` }}
                />
              </div>
              <p className="text-[13px] font-semibold text-[#464552]">
            ...
            <div className="space-y-3">
              {(currentQ.options ?? []).map((opt, i) => {
                ...
                return (
                  <button
                    key={i}
                    onClick={() => { setSelectedOpt(i); if (window.navigator.vibrate) window.navigator.vibrate(10); }}
                  ...
                  >
                    <div
                      className="w-9 h-9 rounded-full flex items-center justify-center font-bold mr-3 shrink-0 text-[13px]"
          ...
          <div className="flex-1 px-5 py-6 flex flex-col gap-5">
            ...
            <div className="flex flex-col items-center py-4">
              ...
              <p className="text-[14px] text-[#464552] text-center mt-1">
                {passed
                  ? "You've demonstrated good understanding of this session."
                  : "Review the material and try again to solidify your knowledge."}
              </p>
            </div>
            ...
            {/* Result message */}
            <div className={`rounded-[24px] p-4 border flex items-center gap-3 ${
              passed
                ? "bg-[#9ef1f2]/20 border-[#006769]/10"
                : "bg-[#ffdad6]/20 border-[#ba1a1a]/10"
            }`}>
              <Icon
                name={passed ? "check_circle" : "info"}
                filled
              ...
              <p className={`text-[13px] font-semibold ${passed ? "text-[#006769]" : "text-[#ba1a1a]"}`}>
                {passed
                  ...
                  : "Session will stay Pending — review and try again."}
              </p>
            </div>
